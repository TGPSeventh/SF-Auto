...
# Go to ${Site} Page
#     [Documentation]                                                    Opens SF Page
#     ${username_status}                                                 Set Variable
#     ...                                                                chrome
#     Maximize Browser Window

Get Site URL
    [Arguments]    ${site}    # Takes the site name as an argument
    ${url}=        Set Variable    ${EMPTY}    # Initialize the URL variable

    # Determine the URL based on the site name (the case here)
    Run Keyword If      '${site}' == 'login'      Set Variable    ${url}    ${SF.forms.loginpage}
    Run Keyword If      '${site}' == 'forgotpw'   Set Variable    ${url}    ${SF.forms.forgotpwurl}
    Run Keyword If      '${site}' == 'homepage'   Set Variable    ${url}    ${SF.forms.homepage}

    Log    Navigating to ${url}    # Log the URL being used
    [Return]             ${url}    # Return the URL based on the case

Go to ${Site} Page
    ${url}=        Get Site URL    ${site}    # Get the URL based on the site
    Go To          ${url}          # Navigate to the URL
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^Sample in TC^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# Home
#     [Documentation]    Navigate to the homepage.
#     Go to ${Site} Page    homepage
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^Sample in TC^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
End Announcement
    ...
    IF    "%{TG_REPORT}" == "True" or "%{TG_REPORT}" == "Full"
        ...
        ...                                                             Salesforce Web Automation Finished Testing${\n}${\n}__${hostname}__
        ...                                                             ${announcement_id}
    END
...
Update Announcement
    ...
    IF    "%{TG_REPORT}" == "True" or "%{TG_REPORT}" == "Full"
        Edit Telegram Message
        ...                                                              Start Testing: *${curr_test_name}*${\n}${\n}Starting Salesforce Web Automation\\.\\.\\.${\n}${\n}__${hostname}__
        ...                                                              ${announcement_id}
    END
...
Append Test Report
    ...
    ELSE
        ${failed}                                                         Set Variable                                 ❌
    END
    Append To List                                                      ${test_report}                               ${TEST_NAME} : ${test}*${TEST_STATUS}*
    ${last_period_index}                                                Evaluate                                     "${SUITE_NAME}".rfind('.')
